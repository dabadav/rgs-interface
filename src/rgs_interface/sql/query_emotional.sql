WITH EmotionalAnswers AS (
    SELECT
        eqp.PATIENT_ID,
        ea.CREATION_TIME AS ANSWER_DATE,
        eq.EMOTIONAL_QUESTION_KEY,
        ea.EMOTIONAL_ANSWER,
    ...
    JOIN emotional_answer as ea
        ON eqp.EMOTIONAL_QUESTION_PATIENT_ID = ea.EMOTIONAL_QUESTION_PATIENT_ID
    JOIN emotional_question as eq
        ON eqp.EMOTIONAL_QUESTION_ID = eq.EMOTIONAL_QUESTION_ID
),
...
SELECT
    ...
    ea1.ANSWER_DATE,
    ea1.EMOTIONAL_QUESTION_KEY AS EMOTIONAL_KEY_1,
    ea2.EMOTIONAL_QUESTION_KEY AS EMOTIONAL_KEY_2,
    ea3.EMOTIONAL_QUESTION_KEY AS EMOTIONAL_KEY_3,
...
LEFT JOIN (SELECT PATIENT_ID, ANSWER_DATE, EMOTIONAL_QUESTION_KEY, EMOTIONAL_ANSWER FROM EmotionalAnswers WHERE answer_rank = 1) ea1
    ON p.PATIENT_ID = ea1.PATIENT_ID AND sd.SESSION_DATE = ea1.ANSWER_DATE
LEFT JOIN (SELECT PATIENT_ID, ANSWER_DATE, EMOTIONAL_QUESTION_KEY, EMOTIONAL_ANSWER FROM EmotionalAnswers WHERE answer_rank = 2) ea2
    ON p.PATIENT_ID = ea2.PATIENT_ID AND sd.SESSION_DATE = ea2.ANSWER_DATE
LEFT JOIN (SELECT PATIENT_ID, ANSWER_DATE, EMOTIONAL_QUESTION_KEY, EMOTIONAL_ANSWER FROM EmotionalAnswers WHERE answer_rank = 3) ea3
